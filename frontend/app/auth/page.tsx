...
export default function AuthPage() {
  ...
  const handleRegisterSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const username = registerEmail.split('@')[0] || `user${Date.now()}`;
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`auth/google/url/?redirect_uri=${encodeURIComponent(redirectUri)}`));
      ...
      setGeneralError(error instanceof Error ? error.message : 'Failed to start Google sign-in');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 relative overflow-hidden p-4">
      <Link href="/" className="absolute top-6 left-6 text-2xl font-bold">
        <span className="text-purple-600">LFG</span>
        <span className="ml-1">🚀</span>
      </Link>

      <div className="w-full bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 relative z-10 max-w-md">
        ...
        {(generalError || loginError || registerError) && (
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors mb-6 disabled:opacity-60"
        >
        ...
        {activeTab === 'login' && (
          <form className="space-y-4" onSubmit={handleLoginSubmit}>
            ...
            <button
              ...
              className="w-full bg-purple-600 text-white py-3 rounded-lg font-medium hover:bg-purple-700 transition-colors shadow-md hover:shadow-lg disabled:opacity-60"
            >
        ...
        {activeTab === 'register' && (
          <form className="space-y-4" onSubmit={handleRegisterSubmit}>
            ...
            <button
              type="submit"
              disabled={registerSubmitting || loading}
              className="w-full bg-purple-600 text-white py-3 rounded-lg font-medium hover:bg-purple-700 transition-colors shadow-md hover:shadow-lg disabled:opacity-60"
            >
