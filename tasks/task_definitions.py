...
from factory.ai_functions import _slugify_project_name
...
"""
    ...
    commands = [
        ...
        f'cd {MAGS_WORKING_DIR}/{project_dir} && git config user.email "{git_email}"',
        f'cd {MAGS_WORKING_DIR}/{project_dir} && git config user.name "{git_name}"',
        # Add all files
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git add -A",
        # Create initial commit
        f'cd {MAGS_WORKING_DIR}/{project_dir} && git commit -m "Initial commit: {stack_name} project initialized by LFG"',
        # Add remote origin (allow to fail if remote already exists)
        f"cd {MAGS_WORKING_DIR}/{project_dir} && (git remote add origin https://{token}@github.com/{owner}/{repo_name}.git || git remote set-url origin https://{token}@github.com/{owner}/{repo_name}.git)",
        # Push main branch to GitHub
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git push -u origin main",
        # Create lfg-agent branch from main
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git checkout -b lfg-agent",
        # Push lfg-agent branch to GitHub
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git push -u origin lfg-agent",
        # Create and checkout feature branch from lfg-agent
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git checkout -b {escaped_branch}",
        # Push feature branch to GitHub
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git push -u origin {escaped_branch}",
        # Verify current branch
        f"cd {MAGS_WORKING_DIR}/{project_dir} && git branch --show-current"
    ]
...
'''
    ...
    result = run_command(job_id, merge_command, timeout=60)
    ...
    if 'CONFLICT' in stdout or 'conflict' in stdout.lower():
        ...
        status_cmd = f"cd {MAGS_WORKING_DIR}/{project_dir} && git status --short | grep '^UU\\|^AA\\|^DD'"
        status_result = run_command(job_id, status_cmd, timeout=30)
        ...
        diff_cmd = f"cd {MAGS_WORKING_DIR}/{project_dir} && git diff --name-status"
        diff_result = run_command(job_id, diff_cmd, timeout=30)
        ...
        try:
            ...
            if 'IMPLEMENTATION_STATUS: COMPLETE' in content:
                ...
                verify_cmd = f"cd {MAGS_WORKING_DIR}/{project_dir} && git status --short | grep '^UU\\|^AA\\|^DD'"
                verify_result = run_command(job_id, verify_cmd, timeout=30)
                ...
                if remaining_conflicts:
                    ...
                    # Abort the merge
                    abort_cmd = f"cd {MAGS_WORKING_DIR}/{project_dir} && git merge --abort"
                    run_command(job_id, abort_cmd, timeout=30)
                    return {
                        'status': 'conflict',
                ...
                push_cmd = f"cd {MAGS_WORKING_DIR}/{project_dir} && git push origin lfg-agent"
                push_result = run_command(job_id, push_cmd, timeout=60)
...
def setup_ticket_workspace(
    ...
    # 2. WORKSPACE PROVISIONING (via Mags — per-ticket workspace forked from claude-auth)
    logger.info(f"[WORKSPACE SETUP] Step 2: Creating Mags workspace for ticket...")
    ...
    try:
        ...
        logger.info(
            "[WORKSPACE SETUP] Resolved base workspace for ticket %s (user=%s): base=%s ticket_ws=%s",
        ...
        workspace, _ = MagpieWorkspace.objects.update_or_create(
            ...
            defaults={
                ...
                'metadata': {'project_name': project.provided_name or project.name},
            }
        )
        logger.info(f"[MAGS][READY] Workspace ready: {ticket_ws} (job {job_id})")
...
def execute_ticket_implementation(ticket_id: int, project_id: int, conversation_id: int, max_execution_time: int = 1200) -> Dict[str, Any]:
    ...
    try:
        ...
        if not project.stack:
            ...
            logger.error(f"[STEP 1/6] ✗ {error_msg}")
...
"""
        ...
        github_token = get_github_token(project.owner)
        # Use project's actual stack (guard above ensures it's set)
        stack = setup_result.get('stack') or project.stack
        stack_config = get_stack_config(stack, project)
        # Get project_dir from stack_config, not hardcoded fallback
        project_dir = setup_result.get('project_dir') or stack_config.get('project_dir', 'project')
        ...
        import re
        files_created = re.findall(r'cat > (project/[\w\-\./]+)', content)
        ...
        if completed and not failed:
            ...
            ticket.last_execution_at = timezone.now()
...
def execute_ticket_with_claude_cli(ticket_id: int, project_id: int, conversation_id: int, max_execution_time: int = 1200) -> Dict[str, Any]:
    ...
    try:
        def _emit_cli_status(message: str) -> None:
            """Push setup progress messages to Ticket Actions panel."""
            try:
                ...
                logger.warning(f"[CLI STATUS] Could not emit setup status log: {e}")

        _emit_cli_status("Preparing ticket execution context...")
        ...
        if not project.stack:
            error_msg = "Cannot execute ticket: project tech stack is not set. Use the product chat to set the stack before executing tickets."
            logger.error(f"[CLI STEP 1/7] ✗ {error_msg}")
            ticket.status = 'failed'
            ticket.save(update_fields=['status'])
            return {
                "status": "error",
        ...
        try:
            ...
            ticket_workspace, _ = MagpieWorkspace.objects.update_or_create(
                ...
                defaults={
                    'project': project,
                    'user': user,
                    'job_id': ticket_ws,         # legacy field (unique) — use workspace name
                    'workspace_id': ticket_ws,   # legacy field (unique) — use workspace name
                    'mags_workspace_id': ticket_ws,
                    'mags_base_workspace_id': base_ws,
...
"""
        ...
        # Setup git in the ticket workspace
        from codebase_index.models import IndexedRepository
        ...
        stack = project.stack
        stack_config = get_stack_config(stack, project)
        project_dir = stack_config.get('project_dir', 'project')
        ...
        if github_owner and github_repo and github_token:
            ...
            logger.info(f"[CLI STEP 3/7] Setting up git repo in ticket workspace...")
            _emit_cli_status("Configuring repository inside sandbox...")
            ...
            git_result = run_command(workspace_id=workspace_id, command=git_setup_script, timeout=120)
...
"""
        ...
        _emit_cli_status("Loading project documentation and building prompt...")
...
❌ IMPLEMENTATION_STATUS: FAILED - [reason]"""
        ...
        def stream_output_callback(new_output: str):
            ...
            for line in lines:
                ...
                if not line:
                    continue
                # Skip status/metadata lines and wrapper control markers
                if (line.startswith('STATUS=') or line.startswith('OUTPUT_SIZE=') or
                    line.startswith('EXIT_CODE=') or line.startswith('___CLAUDE_')):
                    continue
                ...
                if not json_objects and line.startswith('{'):
                    ...
                    continue
                if not json_objects and line:
                    now_ts = time.time()
                    # Log raw non-JSON output at most every 3s and skip duplicates.
                    if (now_ts - last_raw_log_time[0]) >= 3 and line != last_raw_log_line[0]:
                        try:
                            raw_snippet = line[:120].replace('\r', ' ').replace('\n', ' ')
                            raw_log = TicketLog.objects.create(
                                ticket_id=ticket.id,
                                log_type='command',
                                command='CLI output',
                                explanation=f"Agent output: {raw_snippet}",
                                output=line[:4000],
                            )
                            async_to_sync(async_send_ticket_log_notification)(ticket.id, {
                                'id': raw_log.id,
                                'log_type': raw_log.log_type,
                                'command': raw_log.command,
                                'explanation': raw_log.explanation or '',
                                'output': raw_log.output,
                                'created_at': raw_log.created_at.isoformat()
                            })
                            streamed_log_count[0] += 1
                            last_raw_log_time[0] = now_ts
                            last_raw_log_line[0] = line
                        except Exception as raw_err:
                            logger.warning(f"[CLI CALLBACK] Raw output log failed: {raw_err}")
                    continue
                        ...
                        for block in content_blocks:
                            ...
                            if block_type == 'tool_result':
                                ...
                                if result_content and len(result_content) > 50:
                                    result_snippet = result_content.replace('\n', ' ').strip()[:100]
                                    log_entry = TicketLog.objects.create(
                                        ticket_id=ticket.id,
                                        log_type='command',
                                        command='Result' + (' (Error)' if is_error else ''),
                                        explanation=f"{'Error' if is_error else 'Result'}: {result_snippet}",
        ...
        _emit_cli_status("Claude execution finished. Finalizing results...")
...
def execute_ticket_chat_cli(
    ...
    try:
        ...
        try:
            # Verify workspace is accessible via SDK probe.
            # When reusing an existing workspace (previous_ticket_ws is set),
            # do NOT pass base_workspace_id — this avoids run_command calling
            # client.new() which can recreate the workspace from scratch.
            is_reuse = bool(previous_ticket_ws)
            probe_result = run_command(
                workspace_id=ticket_ws,
                command='echo "WORKSPACE_READY"',
            ...
            if 'WORKSPACE_READY' not in probe_result.get('stdout', ''):
                if is_reuse:
                    # Existing workspace is gone — retry with base to recreate
                    logger.info(f"[CLI_CHAT] Workspace {ticket_ws} not responding, recreating from base {base_ws}")
                    probe_result = run_command(
                        workspace_id=ticket_ws,
                        command='echo "WORKSPACE_READY"',
                        timeout=180,
                    ...
                    # Workspace was recreated — old session is gone
                    if session_id:
            ...
            error_log = TicketLog.objects.create(
                ...
                explanation='Could not create workspace',
                output=f'Failed to create workspace: {str(e)}. Please check Settings > Claude Code.'
            )
            async_to_sync(async_send_ticket_log_notification)(ticket_id, {
                'id': error_log.id,
                'log_type': error_log.log_type,
            ...
            return {"status": "error", "error": f"Failed to create workspace: {str(e)}"}
        ...
        try:
            ...
            if 'DIR_MISSING' in dir_stdout or 'DIR_EXISTS' not in dir_stdout:
                ...
                # Project dir was missing — old CLI session is gone too
                if session_id:
        ...
        def stream_callback(new_output: str):
            ...
            # Heartbeat fallback for non-JSON/partial stream chunks.
            try:
                now_ts = time.time()
                if new_output.strip() and (now_ts - last_stream_heartbeat[0]) >= 15:
                    snippet = new_output.replace('\r', ' ').replace('\n', ' ').strip()[:240]
                    if snippet:
                        hb_log = TicketLog.objects.create(
                            ticket_id=ticket_id,
                            log_type='command',
                            command='Agent working',
                            explanation=f"Agent working: {snippet[:120]}",
                            output=snippet,
                        )
                        async_to_sync(async_send_ticket_log_notification)(ticket_id, {
                            'id': hb_log.id,
                            'log_type': hb_log.log_type,
                            'command': hb_log.command,
                            'explanation': hb_log.explanation or '',
                            'output': hb_log.output,
                            'created_at': hb_log.created_at.isoformat()
                        })
                        last_stream_heartbeat[0] = now_ts
            except Exception as hb_err:
                logger.warning(f"[CLI_CHAT] Heartbeat log failed: {hb_err}")
                            ...
                            if isinstance(block, dict) and block.get('type') == 'tool_result':
                                ...
                                if result_content and len(result_content) > 50:
                                    ...
                                    log_entry = TicketLog.objects.create(
                                        ...
                                        log_type='command',
                                        command='Result',
                                        explanation=f"Result: {result_snippet}",
                                        output=result_content
                                    )
                                    try:
                                        async_to_sync(async_send_ticket_log_notification)(ticket_id, {
...
Please respond to the user's message in the context of this ticket."""
        ...
        # Auto-retry with full ticket context + conversation history instead of
        # asking the user to resend their message.
        cli_stdout = cli_result.get('stdout') or ''
        ...
        if session_not_found:
            ...
            recovery_log = TicketLog.objects.create(
                ...
                explanation='Previous session expired, resuming with conversation context...',
                output='Recovering conversation context...'
            )
            async_to_sync(async_send_ticket_log_notification)(ticket_id, {
                'id': recovery_log.id,
                'log_type': recovery_log.log_type,
                'command': recovery_log.command,
                'explanation': recovery_log.explanation,
                'output': recovery_log.output,
                'created_at': recovery_log.created_at.isoformat()
            })

            # Build conversation history from TicketLog entries
            conversation_history = ""
            try:
                recent_logs = TicketLog.objects.filter(
                    ticket_id=ticket_id,
                    log_type__in=['ai_response', 'command', 'system', 'git', 'error']
                ...
                for log in recent_logs:
                    if log.log_type == 'ai_response':
                        ...
                        tool_name = log.command or 'Tool'
                        history_lines.append(f"[Tool: {tool_name}]: {(log.output or '')[:500]}")
                    elif log.log_type == 'system':
                        history_lines.append(f"[System]: {log.explanation or ''} {(log.output or '')[:300]}")
                    elif log.log_type == 'git':
                        history_lines.append(f"[Git]: {(log.output or '')[:300]}")
                    elif log.log_type == 'error':
                        history_lines.append(f"[Error]: {(log.output or '')[:300]}")
            ...
            # Get ticket notes/status updates
            ticket_notes = ""
            try:
                ticket.refresh_from_db()
                note_parts = []
                if ticket.status:
                    note_parts.append(f"Current Status: {ticket.status}")
                if ticket.notes:
                    note_parts.append(f"Notes: {ticket.notes[:1000]}")
                if ticket.github_branch:
                    note_parts.append(f"Branch: {ticket.github_branch}")
                if note_parts:
                    ticket_notes = "\n\nTICKET STATUS:\n" + "\n".join(note_parts)
            except Exception:
                pass

            # Rebuild full prompt with context + history
            stack_info = f"PROJECT STACK: {stack_config['name']}\n" if stack_config else ""
            retry_prompt = f"""You are helping with ticket #{ticket.id}: {ticket.name}
...
Note: The previous conversation session was lost. The conversation history above summarizes what happened before. Please respond to the user's message in the context of this ticket."""

            # Retry with new session
            logger.info(f"[CLI_CHAT] Retrying CLI with full context (no session_id)")
            cli_result = run_claude_cli(
                workspace_id=workspace_id,
                prompt=retry_prompt,
                session_id=None,
        ...
        try:
            ...
            git_status_result = run_command(
                workspace_id=workspace_id,
                command=f"cd {MAGS_WORKING_DIR}/{project_dir} && git status --porcelain",
                timeout=30,
                with_node_env=False,
            )
            ...
            if not git_failed and ticket.github_branch:
                unpushed_result = run_command(
                    workspace_id=workspace_id,
                    command=f"""cd {MAGS_WORKING_DIR}/{project_dir} && git fetch origin 2>/dev/null
...
def ensure_workspace_and_execute(ticket_ids: List[int], project_db_id: int, conversation_id: Optional[int]) -> Dict[str, Any]:
    ...
    broadcast_ticket_notification(conversation_id, {
        ...
        'function_name': 'ticket_execution_queue',
        'status': 'queued',
        'message': "Preparing workspace for ticket execution...",
        'refresh_checklist': bool(ticket_ids)
    })

    # With Mags, per-ticket workspaces are created on demand by each execution function.
    # No need to pre-provision a shared workspace.
    ...
    broadcast_ticket_notification(conversation_id, {
        ...
        'function_name': 'ticket_execution_queue',
        'status': 'completed',
        'message': "Beginning ticket execution...",
