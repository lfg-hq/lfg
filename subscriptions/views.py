...
def init_default_plans(request):
    ...
    try:
        # Define default plans (same as management command)
        default_plans = [
            {
                'name': 'Free Tier',
                'price': 0.00,
        ...
        for plan_data in default_plans:
            ...
            if existing_plan:
                ...
                existing_plan.save()
                plans_updated += 1
                results.append(f"Updated plan: {existing_plan.name}")
            else:
                # Create new plan
                ...
                plans_created += 1
                results.append(f"Created plan: {new_plan.name}")
        
        logger.info(f"Admin {request.user.email} initialized payment plans: {plans_created} created, {plans_updated} updated")
        ...
        return JsonResponse({
            ...
            'results': results,
            'message': f"Successfully created {plans_created} and updated {plans_updated} payment plans."
        })
