document.addEventListener('DOMContentLoaded', () => {
    ...
    // If we landed with a conversation ID (direct URL), pre-load history via REST.
    // WS no longer sends chat_history (to avoid wiping the user's pending message).
    ...
    function handleAIChunk(data) {
        ...
        if (isNotification && !isEarlyNotification) {
            ...
            // ── Ticket streaming: show each ticket in the checklist panel as it's created ──
            if (data.notification_type === 'ticket_stream' && data.ticket) {
                ...
                // Get or create the streaming ticket list container
                let list = document.getElementById('ticket-stream-list');
                if (!list && clPane) {
                    // Hide empty state
                    const empty = clPane.querySelector('.empty-state');
                    if (empty) empty.style.display = 'none';
                    list = document.createElement('div');
                    list.id = 'ticket-stream-list';
                    list.style.cssText = 'padding:12px;display:flex;flex-direction:column;gap:8px;';
                    clPane.appendChild(list);
                }

                if (list) {
                    const priorityColor = t.priority === 'High' ? '#ef4444' : t.priority === 'Low' ? '#22c55e' : '#f59e0b';
                    const card = document.createElement('div');
                    card.style.cssText = 'background:#1e1e2e;border:1px solid #2a2a3e;border-radius:8px;padding:12px 14px;animation:fadeIn 0.3s ease;';
                    card.innerHTML = `
                        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
                            <span style="font-size:13px;font-weight:600;color:#e2e8f0;flex:1;">${t.name}</span>
                            <span style="font-size:11px;font-weight:600;color:${priorityColor};padding:2px 7px;border-radius:4px;border:1px solid ${priorityColor}33;white-space:nowrap;">${t.priority}</span>
                        </div>
                        ${t.description ? `<div style="font-size:12px;color:#9ca3af;margin-top:5px;line-height:1.4;">${t.description.slice(0, 120)}${t.description.length > 120 ? '…' : ''}</div>` : ''}
                        <div style="font-size:11px;color:#6b7280;margin-top:6px;">📋 ${t.stage || 'Backlog'} &nbsp;·&nbsp; ${t.complexity || 'medium'}</div>
                    `;
        ...
        const assistantMessage = document.querySelector('.message.assistant:last-child');
        if (assistantMessage) {
            // Accumulate raw content and render only via marked (no plain-text shortcut — that causes jitter)
