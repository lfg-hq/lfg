...
async def track_token_usage(user: Optional[User], project: Optional[Project], conversation: Optional[Conversation], 
    ...
    try:
        logger.debug(f"track_token_usage called - provider: {provider}, model: {model}, user: {user}, usage_data type: {type(usage_data)}")
        ...
        # Validate we have actual token counts
        if input_tokens == 0 and output_tokens == 0:
            logger.warning(f"Token counts are zero for {provider}. Usage data attributes: {[attr for attr in dir(usage_data) if not attr.startswith('_')]}")
        ...
        logger.debug(f"Calculated cost: ${cost}")
        
        # Save asynchronously
        await asyncio.to_thread(token_usage.save)
        
        logger.info(f"Token usage tracked successfully: {token_usage} (ID: {token_usage.id})")
